...
windspeed <- na.omit(middayweather$wind.speed) + rnorm(15000, sd=0)
plot(windspeed)
write(windspeed, file="windspeed", ncolumns=1)

humidity <- na.omit(middayweather$hum.out) + rnorm(15000, sd=0)
plot(humidity)
write(humidity, file="humidity", ncolumns=1)
